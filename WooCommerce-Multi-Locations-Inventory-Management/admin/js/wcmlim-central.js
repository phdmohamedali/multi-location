...
jQuery( '#Submit_ProdCentral_Control' ).click( function ()
{
  ...
  jQuery.ajax( {
    ...
    success: function ( result )
    {        
      alertify.alert( "<div id='pophead' style='text-align: center;'><span class='dashicons dashicons-yes-alt'></span><h2>Columns Settings!</h2><h4>Updated Control Successfully<h4><div>", function () { location.reload(); } );
    }
  } );
} );
...
jQuery( ".ewc-filter-cat" ).on( "change", function ()
{
  ...
  if ( catFilter != "" )
  {
    document.location.href = "admin.php?page=wcmlim-product-central" + catFilter;
  }
} );
jQuery( ".ewc-filter-type" ).on( "change", function ()
{
  ...
  if ( typeFilter != "" )
  {
    document.location.href = "admin.php?page=wcmlim-product-central" + typeFilter;
  }
} );
jQuery( ".ewc-filter-stock-status" ).on( "change", function ()
{
  ...
  {
    document.location.href = "admin.php?page=wcmlim-product-central" + stockStatusFilter;
  }
} );

  /*
   // example for multiple paramater
  jQuery( 'a#panel_filter_btn' ).on( 'click', function ()
  {
    var filter_form_value = jQuery( 'input#filter_form_value' ).val();
    var panel_filter_select = jQuery( 'select.panel_filter_select' ).val();
    var panel_filter_behavior = jQuery( 'select.panel_filter_behavior' ).val();
    if ( filter_form_value != '' )
    {
      document.location.href = 'admin.php?page=wcmlim-product-central&Srchval=' + filter_form_value + "&by=" + panel_filter_select + "&behavior=" + panel_filter_behavior
    }
  } ); 
  */
 /**
  * Cat and tag for future update
  */
