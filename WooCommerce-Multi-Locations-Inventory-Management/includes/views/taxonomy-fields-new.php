...
?>
  <div class="form-field term-location-priority-wrap">
    ...
    <input class="form-control" id="lcpriority" name="wcmlim_location_priority" type="number" min="1" pattern="[0-9]{10}">
  </div>
<?php }} ?>
  ...
  <div class="form-field term-pos-wrap">
    ...
    <select name="wcmlim_pos_compatiblity" id="wcmlim_pos_compatiblity">
      ...
      foreach ((array) $posts as $p) {
        ...
        // if ($gateway->enabled == 'yes') {
      ?>
        <option value="<?php esc_html_e($p->ID); ?>"><?php esc_html_e($name); ?></option>
      <?php
        // }
      } ?>
    </select>
